...
output_path_price = "outputs\\price.txt"
# if len(args) >= 2:
#     output_path = args[1]
nowtime = time.time()
config = dict()
products = dict()
#读取配置
with open("Goofisher\src\gfc.json", "r", encoding="utf-8") as gfc:
...
try:
    ...
    if final_prices:
        ...
        if output_path_name:
            ...
            for i, (k, v) in enumerate(islice(items, results_show)):
                target1.append(f"{i + 1}:{k}")
                target2.append(f"{i + 1}:{v[0]}")
            with open(output_path_name, "w", encoding="utf-8") as w:
                w.write("\n".join(target1))
            with open(output_path_price, "w", encoding="utf-8") as w:
                w.write("\n".join(target2))
            print(f"名称已保存至“{output_path_name}”文件。")
            print(f"价格已保存至“{output_path_price}”文件。")
        else:
            print(info)
            for i, (k, v) in enumerate(islice(items, results_show)):
