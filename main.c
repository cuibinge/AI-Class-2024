...
void F_value_Sub(int x, int y, int x1, int y1, int x2, int y2, Point Map[ROW][COL])
{
	Map[y][x].F = 0;
	int G, H1, H2;
	H1 = (y - y2) * straight; H2 = (x - x2) * straight;
	G = Get_G(x, y, x1, y1, Map);
	Map[y][x].F = G + (int)sqrt(H1 * H1 + H2 * H2);
}
void print_F(Point Map[ROW][COL])
{
	int i1, i2;
	...
	for (i1 = 1; i1 <= ROW - 1; i1++)
	{
		for (i2 = 1; i2 <= COL - 1; i2++)
		{
			if (Map[i1][i2].list == 'o' && Map[i1][i2].F == -1)
			{
				Map[i1][i2].F = 0;
				int G, H1, H2;
				H1 = (i1 - y2 ) * straight; H2 = (i2 - x2) * straight;
...
Point C_choose(int x2, int y2, Point Map[ROW][COL])
{
...
void Astar_Finding_the_Way(int x1, int y1, int x2, int y2, Point Map[ROW][COL])    //x1,y1 起点; x2,y2 终点
{
	...
	while (Map[y2][x2].list != 'c')
	{
		...
		point = C_choose(x2, y2, Map);
		Tx = point.x; Ty = point.y;
		//print_F(Map);                     //调试用的函数
	}
}
void Clear_way(Point Map[ROW][COL])
{
	int i1, i2;
	for (i1 = 1; i1 <= ROW - 1; i1++)
	{
		for (i2 = 1; i2 <= COL - 1; i2++)
		{
			if (Map[i1][i2].list == 'o' || Map[i1][i2].list == 'c')
			{
				Map[i1][i2].list = 'n';
				Map[i1][i2].F = -1;
			}
...
void Get_final_path(int x1, int y1, int x2, int y2, Point Map[ROW][COL])
{
	...
	for (i = 1; x0 != x1 || y0 != y1; i++)
	{
		...
		x0 = next_x;
		y0 = next_y; 
		//Clear_way(Map);                                       //旧的想法，已放弃
		//Astar_Finding_the_Way(x1, y1, x0, y0, Map);           //同上
	}
	Map[y1][x1].list = 'f';
}
